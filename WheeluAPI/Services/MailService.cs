...
public class MailService : IMailService
{
    ...
    private IDictionary<string, Sender> _senders = new Dictionary<string, Sender>
    {
        {
            "accounts",
            new Sender { Email = "accounts@omiotech.pl", Name = "System kont Wheelu" }
        },
        {
            "applications",
            new Sender
            {
                Email = "applications@omiotech.pl",
                Name = "Wheelu - Platforma Szkół Jazdy",
            }
        },
        {
            "payments",
    ...
    private IDictionary<string, ITemplate> _templates = new Dictionary<string, ITemplate>
    {
        ...
        { "account-recovery", new AccountRecoveryTemplate() },
        ...
        { "instructor-invite-create", new InstructorInviteCreateTemplate() },
        { "instructor-invite-join", new InstructorInviteJoinTemplate() },
        { "instructor-detach", new InstructorDetachTemplate() },
    ...
    public MailService(IConfiguration configuration)
    {
        try
        {
            var connectionString =
                configuration.GetConnectionString("MailTrap")
                ?? throw new NullReferenceException("Missing connection string.");
            var parts = connectionString.Split(";");
            if (parts.Length != 2)
                throw new ArgumentException(
                    "Unexpected format - 'inboxID;accessToken' format required."
                );
