...
export function RenderBreadcrumbs(pathName: string, {rootLink}: IProps, firstLevel: ILevelLeaf[], ...levels:ILevel[]) {
	const result: JSX.Element[] = [];

		const BuildLink = (label: ReactNode, link: string)=>{
			return <Link key={link} component={RouterLink} sx={{display: "flex"}} underline="hover" color="inherit" to={link}>{label}</Link>
		}
		...
		if(location.pathname==rootLink) result.push(BuildLabel(<Home />, "root"));
		...
		for (let i=0; i < parts.length; i++) {
			...
			const source = prevSegment=="root"?firstLevel:levels[i - 1]?.get(prevSegment);

			if(!source) return result;

			const nextSegment = source.find(leaf=>leaf.id==part) ?? source.find(leaf=>leaf.id=="index");
