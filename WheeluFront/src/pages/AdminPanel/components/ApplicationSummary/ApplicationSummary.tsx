...
export default function ApplicationSummary({
	...
	const [rejectReason, setRejectReason] =
		useState<App.Models.ApplicationRejectionReason>("Unspecified");
	...
	const rejectionReasons = useMemo(() => {
		const options: Map<App.Models.ApplicationRejectionReason, string> =
			new Map([
		...
		return Array.from(options.entries()).map((elem) => (
		...
		<Card className={classes.Host} elevation={0}>
			...
			<CardContent>
				...
				<div className={classes.ResolveSection}>
					...
					<Collapse
						classes={{ wrapperInner: classes.RejectionDetails }}
						in={decision == "reject"}
					>
						<TextField
							select
							...
							value={rejectReason}
							onChange={(ev) =>
								setRejectReason(ev.target.value as any)
							}
