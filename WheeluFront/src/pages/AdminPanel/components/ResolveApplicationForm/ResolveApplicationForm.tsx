...
interface IProps {
	...
	onUpdate: (
		data: App.Models.IApplication,
		nearbyCities: App.Models.ICityMatching[]
	) => void;
	onChange: (safeToCommit: boolean) => void;
}

export default function ResolveApplicationForm({
	data,
	cities,
	states,
	onUpdate,
			...
			for (const city of nearbyCitiesList) {
				if (city.cityName == "" || city.state == undefined) {
					if (nearbyCitiesSectionRef.current) {
						const input =
							nearbyCitiesSectionRef.current.querySelector(
								".Mui-error input"
	...
	useEffect(() => {
		...
		for (const city of data.nearbyCities) {
			...
			result.push(
				existingCity
					? {
							identifier: existingCity.id,
							cityName: city,
							state: existingCity.state.name,
					  }
					: { cityName: city }
			);
		}
	...
	const parsedResolvedDate = useMemo(
		() => DateTime.fromISO(data.resolvedAt ?? ""),
		[data.resolvedAt]
	);
		...
		>
			{isReadonly && (
				<Alert severity="info">
					...
					Ten wniosek został rozpatrzony{" "}
					<b>
						{data.status == "accepted"
							? "pozytywnie"
							: "negatywnie"}
					</b>{" "}
					dnia {parsedResolvedDate.toFormat("dd/LL/yyyy")} o godzinie{" "}
					{parsedResolvedDate.toFormat("h:mm")}
				</Alert>
			)}
			...
			<div className={c([classes.InputGroup])}>
				<TextFieldElement
					...
					parseError={(err) =>
						prepareFieldErrorMessage(err, {
							pattern: "Niewłaściwy adres email.",
							maxLength: "125 znaków",
						})
					}
					disabled={isReadonly}
				/>
