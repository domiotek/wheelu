...
export default function SchoolsTable({ supportFilter }: IProps) {
	...
	const { data, isFetching } = useQuery<
		API.School.GetAll.IResponse,
		API.School.GetAll.IEndpoint["error"]
	>({
		queryKey: ["Schools"].concat(
			supportFilter
				? []
				: [
		...
		queryFn: () =>
			callAPI<API.School.GetAll.IEndpoint>("GET", "/api/v1/schools", {
			...
			},
			{
				...
				valueGetter: (_value, row) => {
					return `${row.address.street} ${
						...
						row.address.subBuildingNumber ?? 0 > 0
							? `/${row.address.subBuildingNumber}`
							: ""
					}, ${row.address.zipCode} ${row.address.city}(${
						row.address.state
					})`;
				},
			},
	...
	return (
		<>
			<DataGrid
				...
				onRowDoubleClick={(row) => navigate(`schools/${row.id}`)}
			/>
		</>
	);
}
