...
export default function UsersTable({ supportFilter }: IProps) {
	...
	const { data, isFetching } = useQuery<
		API.User.GetMany.IResponse,
		API.User.GetMany.IEndpoint["error"]
	>({
		queryKey: ["Users"].concat(
			supportFilter
				? []
				: [
		...
		queryFn: () =>
			callAPI<API.User.GetMany.IEndpoint>("GET", "/api/v1/auth/users", {
	...
	const columns = useMemo(() => {
		const result: GridColDef<App.Models.IUser>[] = [
			...
			{
				field: "isActivated",
				headerName: "Aktywowany",
				width: 175,
				type: "boolean",
			},
			{
				field: "lastPasswordChange",
				headerName: "Ostatnia zmiana hasła",
				width: 250,
				type: "dateTime",
				valueGetter: (value) => new Date(value),
			},
